...
export default function EstimateForm() {
  ...
  return state.success ? (
    <Card
      id="thank-you-card"
      className="w-full max-w-md px-6 py-16 bg-background rounded-lg shadow-lg space-y-4"
    >
      <h2 id="thank-you-title" className="text-2xl font-bold text-foreground">
        {tThankYou.title}
      </h2>
      <Alert
        id="thank-you-alert"
        className="bg-green-50 border-green-200 text-primary"
      >
        <AlertDescription id="thank-you-description">
          {tThankYou.description}
        </AlertDescription>
      </Alert>
    </Card>
  ) : (
    <Card className="w-full max-w-md p-6 bg-background rounded-lg shadow-lg">
      ...
      {state.message && !state.success && (
        <Alert
          id="general-error"
          className="mb-4 bg-red-50 border-red-200 text-destructive"
        >
